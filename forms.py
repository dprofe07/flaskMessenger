...
class Form:
    ...
    def __call__(self, *opt_values):
        if len(opt_values) != len(self.options):
            ...
            return self
        new_form = Form([Option(i.name, i.idx, i.placeholder, i.under, i.input_type, i.required, i.value) for i in self.options], self.on_receive, self.title, self.submit, self.hint)
